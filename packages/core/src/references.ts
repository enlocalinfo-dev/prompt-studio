...
export function referenceSummary(bundle: ReferenceBundle | undefined): string {
  if (!bundle) return "";
  const docs = bundle.documents?.filter((d) => d.text?.trim()).length ?? 0;
  const urls = bundle.urls?.filter((u) => u.text?.trim()).length ?? 0;
  if (docs === 0 && urls === 0) return "";
  return `参考資料: PDF ${docs}件 / URL ${urls}件`;
}
